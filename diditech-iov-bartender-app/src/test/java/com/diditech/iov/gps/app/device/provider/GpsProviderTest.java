...
//    @Test
    ...
    @Test
    public void insertDeviceCmd2() {
        String url = String.format("%s/devices/cmd?cmdName=断油电&cmd=SETPRM,RELAY,1&deviceNum=868120251354806", host);
        ...
        ResponseMessage rm = JSON.parseObject(response.body(), ResponseMessage.class);
        Assert.assertNotNull(rm);
        Assert.assertTrue(rm.getData() instanceof String);
    }

    @Test
    public void getDeviceCmd() {
        String url = String.format("%s/devices/cmd?cmdIds=247", host);
        HttpResponse response = HttpUtil.createRequest(Method.GET, url)
                .header("Authorization", token, true)
                .execute();
        log.debug("{}", response);
    ...
    @Test
    public void addSimpleDevice() {
        String url = String.format("http://localhost:8082/devices?deviceNums=%s&clientId=%s&categoryNo=%s", "888888888,1111111", "yiqiyongche", "DO80");
